...
$dice_file = "c:\\University\\workspace\\NewBloomFilter\\dice_name.txt";
...
unlink glob "c:\\University\\workspace\\NewBloomFilter\\Data_Files\\*.txt";
    ...
    {
        @words = split("\"",$line);
        # $words[3] contains first+middle+last name, $words[21] contains ZIP information
        @param = split(" ",$words[3]);
        
        @extraparam = split(" ",$words[21]); # zip[2] has the zip/postal code
        ...
        $combined_name = $param[1]." ".$param[0]." ".$zip;
...
# Level 1 : Create Bigrams for unique names
    ...
    for ($i = 0; $i < $list; $i = $i + 1)
       {
       	...
       	# attach the zip/phone number to the bigram list
       	push @bigrams,$name_var[2];
   ...
   print "$filename contains $total_nlines lines, $total_nwords words.\n"; 
    ...
    my %size_hash = ();
    #% Start looping through the names 
    for (keys %new_hash) {
    ...
    #% inner loop of names for calculating dice coefficient
     for (keys %new_hash) {
