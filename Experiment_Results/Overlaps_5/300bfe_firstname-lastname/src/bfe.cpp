    ...
    for (unsigned i = 0; i < outtrans.size(); ++i){
        float dice_plain; 
                ...
                //if(dice_bfe > dice_plain && a_p >= ceil(a/hashfunctions) && b_p >= ceil(a/hashfunctions) && b_p >= ceil(b/hashfunctions) && c_p <= floor(c/hashfunctions)) { 
                  if(dice_bfe >= dice_plain && a_p >= floor(a/hashfunctions) && a_p <= floor((a+overlaps)/hashfunctions) && b_p >= floor(b/hashfunctions) && b_p <= floor((b+overlaps)/hashfunctions) && c_p <= floor(c/hashfunctions)) {    
...
vector<int> bfe::bigramCount()
{
    ...
    int end = floor((x+overlaps)/hashfunctions);
